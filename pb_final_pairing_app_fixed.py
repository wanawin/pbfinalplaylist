...
def build_ctx(combo_nums, seed_nums, prev_nums):
    ...
    seed_tens = [n // 10 for n in (seed_nums or [])]
    seed_ones = [n % 10 for n in (seed_nums or [])]
    prev_seed_tens = [n // 10 for n in (prev_nums or [])]
    prev_seed_ones = [n % 10 for n in (prev_nums or [])]
    ...
    for path in paths:
        ...
        with open(path, newline="", encoding="utf-8") as f:
            ...
            for raw in reader:
                ...
                row["id"] = (row.get("id") or row.get("fid") or "").strip() or ""
                for key in ("name", "applicable_if", "expression"):
                    if key in row and isinstance(row[key], str):
                        row[key] = row[key].strip().strip('"').strip("'")
                applicable = (row.get("applicable_if") or "True").strip()
                expr = (row.get("expression") or "False").replace("!==", "!=").strip()
                try:
...
# ----------------------
# Seeds (manual or from history file)
# ----------------------
...
def load_history_seeds(history_path="pwrbll.txt"):
    seeds, labels = [], []
    try:
        ...
        for line in lines:
            ...
            if not m:
                continue
            date_numbers, _pb = m.groups()
            parts = date_numbers.split("\t")
            date = parts[0].strip()
            nums = [int(n) for n in re.findall(r"\d{2}", parts[-1])]
            if len(nums) == 5:
                rows.append((date, sorted(nums)))
        rows = rows[::-1]  # chronological
        seeds = [r[1] for r in rows]
        labels = [f"{i+1:02d} | {r[0]} — " + "-".join(f"{x:02d}" for x in r[1]) for i, r in enumerate(rows)]
    except Exception:
        pass
    return seeds, labels
...
def main():
    ...
    default_user_adapted = "pb_final_filters_adapted.csv"
    ...
    if os.path.exists(default_user_adapted):
        filter_paths.append(default_user_adapted)
    ...
    seed_text = st.sidebar.text_input(
        "Seed winner (last draw numbers, 5 nums 1–69, e.g., 01-16-21-47-60)",
        value=""
    ).strip()
    ...
    # History seeds (optional)
    history_seeds, history_labels = load_history_seeds("pwrbll.txt")
    ...
    if use_hist and history_seeds:
        ...
        st.sidebar.caption("Seed set to: " + "-".join(f"{x:02d}" for x in seed_numbers))
    ...
    # Validation messages
    if seed_text and not seed_numbers and not use_hist:
        st.sidebar.error("Seed winner must have exactly 5 integers in 1..69.")
    if prev_text and not prev_numbers and not use_hist:
    ...
    tracked, bad_finals = normalize_final_sets(track_text)
    if bad_finals:
        st.sidebar.warning("Ignored invalid tracked sets: " + ", ".join(bad_finals[:3]) + (" ..." if len(bad_finals) > 3 else ""))
    ...
    for flt in display_filters:
        ...
        default_checked = select_all or flt.get("enabled_default", False)
        label = f"{flt['id']}: {flt.get('name','')} — init cuts {init_counts[flt['id']]}"
    ...
    if tracked:
        ...
        st.download_button(
            ...
            file_name="pb_final_audit_tracked.csv",
            mime="text/csv",
        )
